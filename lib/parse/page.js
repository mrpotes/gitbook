...
function parsePage(src, options) {
    ...
    .map(function(section) {
        ...
        } else if (section.type === 'reveal') {
            var nodes = [], nodeId = 0;
            _.each(section, function(node) {
                if (node.bodyNodes) {
                    var reveal = [], revealNodes = [], revealTitle = node.bodyNodes[0].text;
                    for (var i = 0; i < node.bodyNodes.length; i++) {
                        if (node.bodyNodes[i].type !== 'hr') {
                            revealNodes.push(node.bodyNodes[i]);
                        }
                        if (node.bodyNodes[i].type === 'hr' || i === node.bodyNodes.length - 1) {
                            reveal.push({
                                title: revealTitle,
                                id: reveal.length,
                                content: render(revealNodes, options)
                            });

                            if (i !== node.bodyNodes.length - 1) {
                                revealTitle = node.bodyNodes[++i].text;
                                revealNodes = [node.bodyNodes[i]];
                            }
                        }
                    }
                    nodes.push({ reveal: reveal, id: nodeId++ });
                } else {
                    nodes.push(render([node], options))
                }
            });
            return {
                id: section.id,
